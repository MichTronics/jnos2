...
static void capture_sid (struct mbx *m, char *argv)
{
	...
	*sidcopy = m->stype; strcpy (sidcopy + 1, argv);
	...
	// log (m->user, "SID Capture [%s]", sidcopy);
...
#ifdef	DEBUG
		log (m->user, "SID DB - adding [%s", sidcopy);
#endif
	...
	{
#ifdef	DEBUG
		log (m->user, "SID DB - adding mbox [%s] to [%s", psock, sclptr->sid);
#endif
		/* put it at the top of the list */
		mclptr = malloc (sizeof(J2SIDMBX));

		mclptr->mbox = j2strdup (psock);

		/* time (&(mclptr->when));	13Jun2016, Maiko, log last seen */

#ifdef	NO_MORE_MBOX_FBB_3_JUST_A_STUPID_IDEA

		/* 10Jun2016, Maiko (VE4KLM), Enforce noB2F for non-winlink system */
		if ((Mfbb > 2) && strncmp (sclptr->sid, "WL2K", 4))
		{
			log (m->user, "enforcing noB2F on [%s]", m->name);

			j2AddnoB2F (m->name);	/* add a call to the current list */
		}
#endif
...
int dosidmbxpast (int argc, char **argv, void *p)
{
	...
	for (sclptr = j2sidcaplist ; sclptr; sclptr = sclptr->next)
	{
		tprintf ("[%s", sclptr->sid);

		for (mclptr = sclptr->mbox; mclptr; mclptr = mclptr->next)
		{
			tprintf (" %s %.9s", mclptr->mbox, ctime (&(mclptr->when)) + 11);
