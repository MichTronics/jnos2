...
#ifdef BBS_PASSWD_REQ	/* 19Jan2020, Maiko */

/*
 * 17Dec2019, Maiko (VE4KLM), New password request at bbs prompt,
 * so I'm thinking the best way to do this is just create a new
 * password file, but append with '.req', that way IF there is
 * an existing password already, it's not immediately trashed,
 * and it's up to the sysop to validate and move it over.
 *
 * meaning I will need to add another option to the j2adduser()
 * function from the JNOS Password Management V2 routines, this
 * option will be say 0 for immediate change, or 1 for request,
 * or actually, I could just OR the flag to the type argument
 * instead, then no modifications necessary to other code.
 */
static int doj2passwdV2 (int argc, char *argv[], void *p)
{
	struct mbx *m = (struct mbx *)p;

	if (argc != 3)
	{
		tprintf ("Usage: PA <password> <password>\n *double entry to make sure you get it right :)\n");
	}

	else if (strcmp (argv[1], argv[2]))
		tprintf ("the passwords do not match, nothing done\n");

	else
	{

	/*
	 * the 0x80 tells pwmgr routines to append '.req' to the JNOS
	 * user's password file, then SYSOP can figure out the rest.
	 * 05Jan2020, Maiko, Now have directory and priv paths, right
	 * now I have them set to NULL which write's default values,
	 * all this because I'm getting rid of ftpusers file.
 	 * 15Jan, Maiko, added extern (prototype) declaration
	 * 23Feb2020, Maiko, add new 'gecos' argument, set to NULL here.
	 */
		extern void j2adduser (int, char*, char*, char*, char*, char*);

		j2adduser (0x80, m->name, argv[1], NULL, NULL, 0);

		tprintf ("password request submitted, sysop will be in touch\n");
	}

    return 0;
}

#endif	/* end of BBS_PASSWD_REQ */
...
#endif
#ifdef BBS_PASSWD_REQ	/* 19Jan2020, Maiko */
	/* 17Dec2019, Maiko (VE4KLM) */
    { "password", doj2passwdV2, 0, 0, NULLCHAR },
#endif
