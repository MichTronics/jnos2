...
/* oz2lw - add support for REST <marker> method of restarting xfers */
		...
		if (*arg == '-')
		{
			log (-1, "LIST arguments [%s] not supported, ignoring them", arg);
			*arg = 0;
		}
            file = pathname(ftp.cd,arg);
            if(!permcheck(ftp.path,RETR_CMD,file)) {
...
#endif
    perm = userlogin(ftp->username,pass,&path,FILE_PATH_SIZE,&anony,"ftpperm");
    if (perm == -1)
    {
        usputs(ftp->control,noperm);
        free(path);
...
#ifdef	DONT_COMPILE	/* not yet, kludge for now */

#ifdef GET_PASSWORD_FROM_J2PWMGR
/* 
 * 18Jan2020, Maiko, mixing path and perms was ugly, so when I
 * introduced the new j2pwmgr routines, they broke this badly.
 */
    ftp->perm = perm; /* perms are no longer in the path !!! */
#endif
...
#ifdef GET_PASSWORD_FROM_J2PWMGR
	/*
	 * 18Jan2020, Maiko, This is a damn kludge, I hate doing it
	 *this way, but it's all over the code, perms tacked onto the
	 * end of path, ugly, so do this for starters, fix later.
	 *
	 * 10 bytes is long enough to include permission string
	 */
    ftp->path = malloc (strlen (path) + 10);
    sprintf (ftp->path, "%s\t0x%lx", path, perm);
#else
...
char *file;
{
   ...
   // log (-1, "permcheck path [%s] op %X file [%s]", path, op, file);
