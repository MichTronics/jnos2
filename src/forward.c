...
void *v1, *v2;
{
	...
	char MyCall[AXBUF], *mcptr, *crstr = NULL;
#endif
    cc = (char*)v1;
    if ((m = newmbx ()) == NULLMBX)
	{
 ...
 */
		...
		if (strnicmp (m->line, ";PQ: ", 5) == 0)
		{
			...
			extern char *j2userpasswdV1 (int, char*);
			...
			*mcptr=0;
			...
			log (m->user, "[%s] requesting secure login", MyCall);

			/* 26Feb2020, Maiko, Error check was missing, need to have this !!! */
			if ((mcptr = j2userpasswdV1 (1, MyCall)) == (char*)0)
			{
