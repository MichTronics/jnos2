...
/*
 * 28Nov2019, Maiko (VE4KLM), password no longer stored in ftpusers.
 *
 * Use the new script 'importFTPUSERSpasswords.sh' to convert your
 * existing ftpusers to the new format, and create the corresponding
 * entries in the JNOS 2.0 Password Manager Database.
 *
 * Instructions can be found at the very top of the script file.
 *
 * You must add the following to your config.h when compiling :
 *
 *    #define GET_PASSWORD_FROM_J2PWMGR
 *
 * or else JNOS will continue to get passwords from ftpusers.
 *
 */

#define GET_PASSWORD_FROM_J2PWMGR	/* 03Dec2019, Maiko, testing */
...
/* 15Jan2020, Maiko */
#ifdef	GET_PASSWORD_FROM_J2PWMGR
extern int j2chkuser (int, char*, char*, char**, char**);
#ifdef MD5AUTHENTICATE
/* 21Feb2020, Maiko */
extern char *j2chkmd5V1 (char*);
...
char *defname;
{
#ifdef	GET_PASSWORD_FROM_J2PWMGR	/* 11Jan2020, Maiko, Compiler warnings */
    int password_okay;
#else
    char *line;
    FILE *fp;
...
#endif
    long perm;
    char *user,*password,*privs,*directory, *cp;
    char *universal = NULLCHAR;
    char *defalt = NULLCHAR;
#ifdef MD5AUTHENTICATE
    int32 challenge;
...
#ifndef	GET_PASSWORD_FROM_J2PWMGR	/* 04Jan2020, Maiko, No more FTPUSERS file ! */

/* NOTE : 04Jan2020, still have to delete with anonymous and universal ? */
...
#endif	/* end of #ifndef GET_PASSWORD_FROM_J2PWMGR */

#ifdef	GET_PASSWORD_FROM_J2PWMGR

	/*
	 * 03Dec2019, Maiko (VE4KLM), Deciding to go direct to j2pwmgrV2
	 *  (no more password storage, using HASH:SALT pairs now) 
	 *
	 * Some challenges have already appeared, for instance the existing
	 * WL2K challenge code, as well as the JNOS MD5AUTHENTICATE require
	 * a cleartext password to work, so will need to continue using the
	 * original j2pwmgr code (renamed to j2pwmgrV1) to deal with that,
	 * using some other type of encryption, since libcrypt is no more.
	 *
	 * 04Jan2020, Maiko, Decided a while ago to just completely remove
	 * the ftpusers file, and just contain everything to the j2pwmgrV2
	 * functions, so we need to add privs and directory to j2chkuser.
	 *
	 * MAKE SURE to free directory and privs ptrs after use !
	 */
	password_okay = j2chkuser (0, name, pass, &directory, &privs);

	/*
	 * 11Jan2020, Maiko, Oops ! If the user does not exist, then check if
	 * the passed default user <defnam> exists, ie: "tcpperm", "ax25perm",
	 * or whatever. If not, then check for "univperm" as a last resort.
	 */
	if (password_okay == -2)
	{
		defalt = j2strdup (defname);

		/* check for default user */
		password_okay = j2chkuser (0, defalt, "nopassword", &directory, &privs);

		if (password_okay == -2)	/* user not found */
		{
			universal = j2strdup ("univperm");

			/* check for universal user */
			password_okay = j2chkuser (0, universal, "nopassword", &directory, &privs);
			...
			else if (password_okay)	/* User ID is password-free */
			{
				log (-1, "univperm - no password required");
				*pwdignore = 1;
			}
			else
			{
				/* if we get here, it means univperm exists, but a valid password is required */

				log (-1, "univperm - password required");
				password_okay = j2chkuser (0, universal, pass, &directory, &privs);
			}
		}
		else if (password_okay)	/* User ID is password-free */
		{
			log (-1, "%s - no password required", defalt);
			*pwdignore = 1;
		}
		else
		{
			/* if we get here, it means <defnam> exists, but a valid password is required */

			log (-1, "%s - password required", defalt);
			password_okay = j2chkuser (0, defalt, pass, &directory, &privs);
		}
	}
/*
 * 04Jan2020, Maiko, MD5AUTHENTICATE is NOT possible with the new J2pwmgrV2
 * routines, since we need the passwords in cleartext, which is not happening
 * now that we are using hash:salt values instead. So those requiring the MD5
 * AUTHENTICATE feature will have to stick with ftpusers for now !
 *
 * 21Feb2020, Maiko, Okay, now accommodating MD5AUTHENTICATE with a new
 * type = 3 user file, don't like using encrypted passwords, but can not
 * just leave people out in the cold, so created a new j2chkmd5() func.
 *
 */
	if (!(*pwdignore) && !password_okay)
	{
...
#endif
	/*
	 * 21Feb2020, Maiko (VE4KLM), Newly introducted j2chkmd5V1() function, to
	 * to get password from recent j2pwmgrV2 libs so MD5AUTHENTICATE continues
	 * to work for people. Make sure to free (password) after it's used !!!
	 *
	 * 26Feb2020, Maiko, Error checking should be present on this.
	 *
 	 */
			if ((password = j2chkmd5V1 (name)) == (char*)0)
			{
				log (-1, "error - no md5 entry for [%s]", name);
			...
			free (password);	/* you must free it after you're done with it */
...
#endif	/* 04Jan2020, Maiko, No more ftpusers files */
...
#ifdef	GET_PASSWORD_FROM_J2PWMGR
	/* 04Jan2020, Maiko, Free pointers we got from j2chkuser() call */
    free(directory);
    free(privs);
#endif
 
	log (-1, "%s perms %lx (%ld)", name, perm, perm);
